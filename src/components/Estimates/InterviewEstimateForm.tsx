...
interface InterviewEstimateFormProps {
  isOpen: boolean;
  onClose: () => void;
  customerId?: number;
  customerName?: string;
...
const InterviewEstimateForm: React.FC<InterviewEstimateFormProps> = ({ 
  isOpen, 
  onClose, 
  customerId, 
  customerName 
}) => {
  const { user, customers, workOrders, createEstimate, notifyEstimateCreated, inventoryList } = useAuth();
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Which customer is this estimate for?
              </label>
              <select
      ...
      case 2:
        return (
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                What job is this estimate for? (Optional)
              </label>
              <select
                value={formData.job}
      ...
      case 3:
        return (
         ...
         return (
           <div className="space-y-6">
             <div className="flex justify-between items-center">
               <label className="block text-sm font-medium text-gray-700">
                 Add inventory items to this estimate?
               </label>
             </div>
             ...
             <div className="space-y-4">
               ...
               {(!inventoryList || inventoryList.length === 0) && (
                 <div className="text-center py-8">
                   ...
                   <p className="text-gray-400">Add inventory items to include them in estimates.</p>
                 </div>
               )}
             </div>
           </div>
         ...
         return (
           <div className="space-y-6">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div>
                 <label className="block text-sm font-medium text-gray-700 mb-2">
                   When does this estimate expire?
                 </label>
                 <input
                   type="date"
           ...
           <div className="space-y-6">
             <div>
               ...
               <textarea
                 ...
                 rows={4}
                 className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                 placeholder="Payment terms, warranty information, or special conditions..."
               />
             </div>
           </div>
         );
       ...
       case 7:
        ...
        return (
          <div className="space-y-6">
            <div className="bg-gray-50 rounded-lg p-4">
              <h3 className="font-semibold text-gray-900 mb-4">Estimate Summary</h3>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const success = await createEstimate(estimateData);
      if (success) {
        notifyEstimateCreated(formData.description);
        onClose();
      }
    } catch (error) {
          ...
          <div className="flex items-center space-x-3 mb-6">
            ...
            <div>
              <h3 className="text-lg font-medium text-gray-900">{getStepTitle(currentStep)}</h3>
                             <p className="text-sm text-gray-600">
                 {currentStep === 1 && 'Select the customer for this estimate'}
                 ...
                 {currentStep === 5 && 'Set expiration date and pricing terms'}
                 {currentStep === 6 && 'Add any additional notes or terms'}
...
export default InterviewEstimateForm;
