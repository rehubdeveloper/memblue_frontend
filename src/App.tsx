...
const AppLayout: React.FC<AppLayoutProps> = ({ showLogin, setShowLogin }) => {
  ...
  const isAdminLike = ['admin', 'solo'].includes(currentUser?.role ?? '');
  const [activeTab, setActiveTab] = useState(
    isAdminLike ? 'dashboard' : 'mobile-dashboard'
  );
    ...
    if (isAdminLike) {
      switch (activeTab) {
        ...
        case 'schedule':
          return <ScheduleView />;
        case 'jobs':
          return <JobsList onNewJob={() => setShowJobForm(true)} />;
        case 'customers':
          return <CustomersList />;
        case 'estimates':
          return <EstimatesInvoices onNewEstimate={() => setShowEstimateForm(true)} />;
        case 'inventory':
      ...
      switch (activeTab) {
        case 'mobile-dashboard':
          return <MobileDashboard currentUserId={String(currentUser.id)} />;
        case 'my-jobs':
          return <JobsList onNewJob={() => setShowJobForm(true)} />;
        case 'schedule':
          return <ScheduleView />;
